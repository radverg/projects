...
namespace TankHunt
{
    ...
    /// </summary>
    public class TankHunt : Microsoft.Xna.Framework.Game
    {
        // Graphics
        ...
        public static SoundEffect avi_kungfu;
        ...
        {
            Background_color = new Color(244, 248, 251);
            try
            {
               // CheckForUpdates();
            }
            ...
            // Load pictures to form
           
            
            try // Load settings
            {
                settings = new Settings(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "TankHunt", "settings") + ".dat");
            ...
            catch
            {
                ...
                settings.ChangeResolution(GraphicsAdapter.DefaultAdapter.CurrentDisplayMode.Width, GraphicsAdapter.DefaultAdapter.CurrentDisplayMode.Height);
            ...
            Set_screen(settings.Screen_width, settings.Screen_height, false, false); // Sets screen
            ...
            container.Player_tank_c = new PlayerTankComponent(this);
            ...
            container.Darkness_c = new DarknessComponent(this);
            // -----------------------------------------------------------------------------------------------------------------
            
            // Creates multiplayer screens -------------------------------------------------------------------------------------
            PlayingMScreen = new GameScreen(this, container.Network_c, container.Srl_c, container.Mshots_c, container.Items_c, container.Player_tank_c, container.Darkness_c, container.Screen_in_c);
        ...
        protected override void LoadContent()
        {
            ...
            try
            {
                ...
                avi_kungfu = Content.Load<SoundEffect>("Sounds\\avikungfu");
                base.LoadContent();
            }
            catch (Exception e)
            {
        ...
        public void Set_screen(int width, int height, bool fullscreen, bool mouse)
        {
            ...
            SC.screen_rectangle = new Rectangle(0, 0, graphics.PreferredBackBufferWidth, graphics.PreferredBackBufferHeight);
            SC.screen_center = new Vector2(SC.screen_rectangle.Width / 2, SC.screen_rectangle.Height / 2);
            SC.res_ratio = width / 1920.0f;
        ...
        {
            SC.Game_paused = true;
            bool was_fs = graphics.IsFullScreen;
            if (graphics.IsFullScreen)
                graphics.ToggleFullScreen();
            ConnectionInfo connect_info = con_info;
            if (connect_info == null)
               connect_info = new ConnectionInfo();
            ConnectionForm form = new ConnectionForm(connect_info, settings);
            GameWindowForm.Hide();
            ...
            if (graphics.IsFullScreen != was_fs)
                graphics.ToggleFullScreen();
